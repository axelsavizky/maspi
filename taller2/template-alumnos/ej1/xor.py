...
J       = np.zeros(epochs)
J[0]    = 1e3
deltas_k = np.zeros(epochs)
deltas_j = np.zeros(epochs)
m       = 0
while m < epochs:
    for i in range(M):
        ...
        aj				=  Wh @ Xm + bh * np.ones((Nh, 1)) ##%% ESCRIBIR EL CODIGO AQUI
        [y, dfh]		= activation(aj)
        ...
        delta_k		= (tk-zk)*dfo ##%% ESCRIBIR EL CODIGO AQUI
        #...y delta_j: 
        delta_j		=  dfh * (Wo.T @ delta_k)## %% ESCRIBIR EL CODIGO AQUI
        ...
        Wh				= Wh + eta * delta_j @ Xm.T ##; %% ESCRIBIR CODIGO AQUI
    ...
    J[m] = J[m]/M
    print('Iteracion %d: delta_k: %s delta_j: %s Wh: %s Wo: %s Error Total %f' % (m, delta_k, delta_j, Wh, Wo, J[m]))
    m = m + 1
